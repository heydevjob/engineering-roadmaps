...
The questions below are the ones that actually decide data engineering interviews - the SQL traps, the ETL reliability gotchas, the modeling judgment calls. They follow the [Data Engineer Roadmap](../roadmaps/data.md) from junior SQL up to senior warehouse design. Each answer is what a hiring manager wants to hear: correct, tight, and grounded in real pipelines.

Reading the answer is step one. Shipping the fix in a real broken system is what gets you hired. Where a question maps to a HeyDevJob ticket, there's a "Practice it live" link - go fix the actual thing in a cloud workspace.

---

## Stage 1 - SQL Foundations

**Why does a one-to-many join inflate a `SUM`?**
...
Practice it live: [Debug a Broken SQL Revenue Aggregation](https://heydevjob.com/data)
...
**Explain `LEFT JOIN` vs `INNER JOIN` and a common bug.**
...
Practice it live: [Rank Rows Per Group With Window Functions](https://heydevjob.com/data)
...
**What does idempotency mean for an ETL load, and how do you achieve it?**
...
Practice it live: [Make an ETL Pipeline Idempotent](https://heydevjob.com/data)

**How should a pipeline handle malformed input rows?**
...
Practice it live: [Make a CSV Importer Resilient to Bad Data](https://heydevjob.com/data)

**Where do you put validation - at ingestion or after transformation?**
...
**A nightly job failed at 2am. How do you make the pipeline survive that?**
...
**What is a watermark and why is incremental loading better than truncate-and-reload?**
...
Practice it live: [Implement Incremental Loading in an ETL](https://heydevjob.com/data)

**What can go wrong with a timestamp-based watermark?**
...
**Why convert CSV to Parquet, and what does columnar storage buy you?**
...
Practice it live: [Convert CSV to Parquet for a Lakehouse](https://heydevjob.com/data)

**What is partition pruning and how does Hive-style partitioning enable it?**
...
**Why are row-by-row inserts slow, and what do you replace them with?**
...
Practice it live: [Optimize Batch Insert Throughput (COPY)](https://heydevjob.com/data)

**When would you reach for Polars over pandas?**
...
Practice it live: [Migrate a pandas Pipeline to Polars](https://heydevjob.com/data)

**A query that was fast is now slow. How do you diagnose it?**
...
**Why use Airflow instead of cron plus a Python script?**
...
Practice it live: [Schedule a Pipeline as an Airflow DAG](https://heydevjob.com/data)

**What does it mean for an Airflow task to be idempotent, and why does it matter for backfills?**
...
**Explain the dbt staging-to-marts pattern.**
...
Practice it live: [Build a Staging-to-Marts dbt Project](https://heydevjob.com/data)

**What's the difference between a dbt source and a ref, and why use them?**
...
**What kinds of tests does dbt give you and what do they catch?**
...
**What is a Kimball star schema and why is it still the default for warehouses?**
...
Practice it live: [Build a Kimball Star Schema](https://heydevjob.com/data)

**Why use surrogate keys instead of natural keys in a dimension?**
...
**How do you back-fill a new column on a large table safely?**
...
Practice it live: [Backfill a Column Safely (Batched + Verified)](https://heydevjob.com/data)

**Kafka delivers at-least-once by default. How do you get exactly-once processing?**
...
Practice it live: [Build an Exactly-Once Kafka Pipeline](https://heydevjob.com/data)

**How do you handle a slowly changing dimension (SCD Type 2)?**
