...
>
> Work top to bottom. You learn to talk to a model before you build a system around it. The work types: **Build** (create the feature from nothing), **Debug** (find and fix a broken LLM integration), **Tune** (improve prompt or retrieval quality), **Optimize** (cut cost and latency), **Design** (architect a RAG/agent/eval system).

```mermaid
flowchart TD
    S1["Stage 1 · LLM API Integration"] --> S2["Stage 2 · Structured Output"]
    S2 --> S3["Stage 3 · Building LLM Endpoints"]
    S3 --> S4["Stage 4 · Prompt Engineering"]
    S4 --> S5["Stage 5 · Stateful &amp; Streaming Chat"]
    S5 --> S6["Stage 6 · RAG: Retrieval"]
    S6 --> S7["Stage 7 · Agents &amp; Tools"]
    S7 --> S8["Stage 8 · Cost &amp; Performance"]
    S8 --> S9["Stage 9 · Senior: RAG Systems, Eval &amp; Agents"]
...
**Explore AI/ML** · [📍 Roadmap](ai.md) · [🛠️ Projects](../projects/ai/README.md) · [💬 Interview](../interview/ai.md) · [✅ Checklist](../checklists/ai.md)
